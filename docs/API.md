...
`question` 去除首尾空白后 1–2000 字符，单个补充最多 2000 字符，会话补充累计最多 8000 字符。关注点最多两项，字段选择只允许当前卡片提供的字段。`changes` 可编辑所有条件字段，传 `null` 清除可选字段。`purpose`：`undergraduate`、`postgraduate`、`campus`、`overview`；`priorities` 为字符串数组。
