...
| ------------------ | --------------- | ----------------------------------- | -------------------------------- |
| 知乎经验与观点     | `search zhihu`  | GET `/api/v1/content/zhihu_search`  | 已实现 HTTP 适配、受控测试       |
| 外部事实补充       | `search global` | GET `/api/v1/content/global_search` | 已实现 HTTP 适配、受控测试       |
| 整理有来源的回答   | `answer`        | POST `/v1/chat/completions`         | 已实现 HTTP 适配，真实调用待验证 |
