...
4. 生成 0 到 5 个简短、互不重叠、与该追问直接相关的单选回答；数值预算等适合自由输入的题可用空数组。选项只是建议，不要求用户从中选。不要使用通用的成本、时间、风险六维度模板。
...
追问 JSON 示例：{"action":"ask","question":"你主要用相机拍什么？","description":"拍摄对象会影响机身和镜头的选择。","options":["旅行风景","人像日常","运动或野生动物","视频拍摄"],"placeholder":"也可以描述你最常拍的场景"}
