...
const agentLibrary = [
  { id: 'guest-evidence', name: '证据核验员' },
  { id: 'guest-counter', name: '反方追问者' },
  { id: 'guest-context', name: '背景补充员' },
];
const guestSpots = [
  { x: 32, y: 82 },
  { x: 69, y: 82 },
  ...
  return (
    <section className="room-experience" aria-label="可视化圆桌">
      ...
      >
        ...
        {round.roles.filter(role => role.kind !== "guest").slice(0, 4).map((role, i) => {
          const seat = seats[i],
            target = podiums[i],
            active = role.id === message?.speaker;
      ...
      <div className="room-caption" aria-live="polite">
        <span>
          {round.messages.length ? "主持人已安排 · " : "会场已就绪，发起一个问题开始讨论"}
          {round.roles.find((r) => r.id === message?.speaker)?.name}
