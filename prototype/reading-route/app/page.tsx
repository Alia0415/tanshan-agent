...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/api/discovery?view=" + view + (searchTerm ? "&q=" + encodeURIComponent(searchTerm) : ""), {signal: controller.signal})
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="topbar-inner">
      <Link className="brand" href="/"><span className="zhihu-word">知乎</span><span className="brand-divider"/><span className="tanshan-word">探山</span></Link>
    ...
    <main className="feed-page">
      <div className="feed-intro"><span>今天，大家在讨论什么</span><h1>从一个好问题，开始阅读。</h1><p>浏览问题，进入回答区。热门讨论里，探山帮你找到值得先读的方向。</p></div>
      <div className="feed-layout"><section className="question-feed" aria-label="问题列表">
        ...
        {loading ? <p className="feed-notice" role="status">正在加载真实知乎问题…</p> : error ? <div className="feed-notice" role="alert"><p>{error}</p><button className="secondary-button" onClick={() => browse(searchTerm, view)}>重试</button></div> : questions.length === 0 ? <p className="feed-notice">没有找到问题，换一个关键词试试。</p> :
          questions.map(item => <article className="feed-question" key={item.id}>
            <div className="feed-question-meta"><span>{searchTerm ? "知乎问题" : view === "history" ? "曾被探山记录在榜" : "热榜第 " + item.rank + " 位"}</span>{item.firstSeenHot && <span className="assistant-badge">可用探山阅读</span>}</div>
            <h2><Link prefetch={false} href={"/question/" + item.id}>{item.title}</Link></h2>
            {item.summary && <p>{item.summary}</p>}
            <Link prefetch={false} className="question-entry" href={"/question/" + item.id}>进入问题，查看回答 <span aria-hidden="true">→</span></Link>
          </article>)}
      </section><aside className="discovery-note"><span className="agent-avatar">山</span><h2>热门问题，慢慢读懂</h2><p>进入问题后，先告诉探山你关注什么，再沿着分类地图阅读。</p><p>已记录上榜的问题会保留阅读助手入口。</p><small>这里是探山独立演示站，内容来自知乎。</small></aside></div>
    </main>
  </div>;
}
