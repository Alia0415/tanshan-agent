...
import type { Question } from "@/lib/discovery";
...
export default function QuestionReader({ question }: { question: Question }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/questions/" + question.id + "/answers", {signal: controller.signal})
  ...
  async function loadClarifyingQuestion(nextHistory: ReadingTurn[] = []) {
    ...
    setApiError("");
    try {
      const response = await fetch("/api/clarify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ questionId: question.id, history: nextHistory }) });
  ...
  async function generateMap() {
    ...
    try {
      const response = await fetch("/api/reading-map", {
        method: "POST",
    ...
    <div className="app-shell">
      <header className="topbar">
        <div className="topbar-inner">
          <Link className="brand" href="/" aria-label="返回問山首页">
      ...
      <main id="top" className="page-grid">
          ...
          {assistantEnabled && <section className={`agent-surface stage-${stage}`} aria-live="polite">
            ...
            {stage === "entry" && (
              <div className="entry-panel">
                <div><h2>回答很多，不知道先看什么？</h2><p>回答几个简单问题，让 AI 按你的处境整理一张阅读地图。其他观点不会被隐藏。</p></div>
                <button className="primary-button" type="button" onClick={startReading}>生成我的阅读地图</button>
                <span className="time-note">约 30 秒 · 可随时跳过</span>
              </div>
            )}
            ...
            {stage === "summary" && (
              <div className="summary-panel">
                ...
                <p>我们会按你的关注点整理四个阅读方向，每个方向提供 4–5 篇相关内容。</p>
                <div className="panel-actions summary-actions">
                  ...
                  <button className="primary-button" type="button" disabled={isGenerating} onClick={generateMap}>{isGenerating ? "正在生成分类并检索…" : "用真实知乎内容生成"}</button>
                </div>
            ...
            {stage === "map" && (
              <div className="map-panel">
                <div className="map-intro">
                  <div><span className="map-status"><i /> 来自知乎开放平台的真实内容</span><h2>先选阅读方向，再进入这一类的具体帖子</h2><p>{mapSummary}</p></div>
                        ...
                        <div className="route-leg" key={category.id} role="listitem">
                          ...
                          >
                            ...
                            <span className="route-post">
                              ...
                              <b>{category.posts.length} 篇真实知乎帖子</b>
                ...
                <div className="route-legend"><span><i className="legend-blue" /> 数字代表分类阅读顺序</span><span><i className="legend-white" /> 每类包含 4–5 篇真实内容</span></div>
                <p className="source-disclaimer">分类包含同一话题下的相关问题与文章，不限于当前问题的回答。预览为搜索摘要，完整语境请阅读原文。</p>
              </div>
            )}
          </section>}
        ...
        <aside className="side-column" aria-label="阅读提示">
          ...
          <div className="side-note"><strong>AI 负责带路</strong><p>观点仍来自真实创作者。摘要帮助你判断是否值得打开，原回答保留完整语境。</p></div>
        </aside>
      </main>
        ...
        <div className="post-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedPost(null); }}>
          <article className="post-dialog" role="dialog" aria-modal="true" aria-labelledby="post-title">
            ...
            <div className="post-dialog-body">
              <span className="demo-badge">知乎真实{selectedPost.contentType === "Article" ? "文章" : "回答"}</span>
            ...
            <footer className="post-dialog-footer">
              <span>继续阅读将打开知乎原文，保留作者的完整表达与讨论语境。</span>
